...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // copying line in an other string
            int lineLeng = strlen(line);
            char toParse[lineLeng + 1];
            strcpy(toParse, line);

            // storing Method, request-target, and HTP-version in requElems
            char* requElems[3];
            char* token;
            char* str1;
            int j;
            for (j = 1, str1 = toParse; ; j++, str1 = NULL)
            {
                ...
                requElems[j - 1] = malloc(sizeof(char) * strlen(token) + 1);
                strcpy(requElems[j - 1], token);
            }

            // checking the CRLF
            int lastChunkLen = strlen(requElems[2]);
            char CRLF[3];
            CRLF[0] = requElems[2][lastChunkLen - 2];
            CRLF[1] = requElems[2][lastChunkLen - 1];
            CRLF[2] = '\0';
            if (strcmp(CRLF, "\r\n") != 0)
            {
                error(400);
            ...
            // requElemes[1] contains request-target            
            int requestLength = strlen(requElems[1]);
            ...
            // determining the length of query and path
            int pathLength;
            int queryLength;
            ...
            if (wantedIndex == -1)
            {   
                // copying the rest of path
                for (int i = 0; i < requestLength; i++)
                {
                    path[j] = requElems[1][i];
                    j++;
                }
                path[pathLength - 1] = '\0';
                
                // query will have a value of ""
                strcpy(query, "\0");
            }
            ...
            {   
                // copying the rest of path
                for (int i = 0; i < (requestLength - 1); i++)
                {
                    path[j] = requElems[1][i];
                    j++;
                }
            ...
            {   
                ...
                // copying after '?' until the end of request-target
                j = 0;
                for (int i = wantedIndex + 1; i < requestLength; i++)
                {
                    query[j] = requElems[1][i];
                    j++;
                }
                query[j] = '\0';
            }

            // freeing allocated space for request elements
            j = 0;
            while (j < 3)
            {
                free(requElems[j]);
            ...
            int extensionLength = pathLength - periodIndex;
            char extension[extensionLength];
            j = 0;
            for (int k = periodIndex + 1; k < pathLength; k++)
            {
                extension[j] = path[k];
                j++;
            }
            ...
            {
                // open pipe to PHP lowerExtpreter
...
{
    // extension length 
    int extenLength = strlen(extension);
    
    // all to lowercase   
    char inter[extenLength + 1];
    for (int i = 0; i < extenLength; i++)
    {
