...
/**
 * arrayAllocation() allocates an array of nodes, initialize all
 * its boolean value to false, and all its child pointers to NULL.
 */
node* arrayAllocation(void)
{
    // 26 alphabetic characters and 1 '
    node* array = malloc(sizeof(node) * 27);
    if (array == NULL)
    {
        return NULL;
    }

    // initialize bool to false, node* to NULL
    for (int i = 0; i < 27; i++)
    {
        array[i]->isWord = false;
        array[i]->child = NULL;
    }
    ...
    {   
        ...
        {   
            ...
            if (isNewWord)
            {
                ...
                isNewWord = false;
                cursor = cursor->child[tolower(c) - 'a'];
            }
